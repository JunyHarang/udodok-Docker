...
public class NationwidelibraryInfoService {
    ...
    // 전국 도서관 정보 삭제 Method
    public DefaultRes deleteLibraryInfo(Long nationwideLibraryInfolibraryInfoId) {
        Optional<NationwideLibraryInfo> libraryInfoOptional = nationwidelibraryInfoQueryRepository.findByLibraryId(nationwideLibraryInfolibraryInfoId);

        if (libraryInfoOptional.isEmpty()) { // 대상 게시물이 없다면?
            return DefaultRes.response(HttpStatus.OK.value(), "내용없음");
        } // if문 끝

        return libraryInfoOptional.filter(nationwideLibraryInfo -> nationwideLibraryInfo.getLibraryInfoId().equals(nationwideLibraryInfolibraryInfoId)).map(nationwideLibraryInfo -> {
            nationwidelibraryInfoQueryRepository.deleteByLibraryId(nationwideLibraryInfo.getLibraryInfoId());
